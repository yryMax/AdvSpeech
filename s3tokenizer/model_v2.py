...
from s3tokenizer.model import Conv1d
from s3tokenizer.model import LayerNorm
from s3tokenizer.model import Linear
from s3tokenizer.model import MultiHeadAttention
from s3tokenizer.utils import make_non_pad_mask
...
class FSQCodebook(torch.nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        powers = torch.pow(
            self.level, torch.arange(2**self.level, device=x.device, dtype=h.dtype)
        )
