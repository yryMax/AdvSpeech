...
class CheapTrick(nn.Module):
    def __init__(self, fs: int, q1: float = -0.15, f0_floor: float = 71.0, default_f0: float = 500.0):
        super().__init__()
        self.fs = fs
        self.q1 = q1
    ...
    def forward(self, x: torch.Tensor, f0: torch.Tensor, temporal_positions: torch.Tensor) -> torch.Tensor:
        ...
        spectrogram = torch.zeros((fft_size // 2 + 1, num_frames), dtype=x.dtype, device=x.device)

        for i in range(num_frames):
            spectrogram[:, i] = self.estimate_one_slice(x, f0[i], temporal_positions[i], fft_size, self.q1)

        return spectrogram

    def estimate_one_slice(self, x: torch.Tensor, current_f0: torch.Tensor, current_position: torch.Tensor,
                           fft_size: torch.Tensor, q1: float) -> torch.Tensor:
        ...
        base_index = torch.arange(-half_window_length, half_window_length + 1, device=x.device, dtype=x.dtype)
        ...
        frequency_axis = torch.arange(0, fft_size_val, device=x.device, dtype=x.dtype) / fft_size_val * fs_t
        ...
        low_frequency_replica = self.interp1H_torch(x_in_reversed, y_in_reversed, query_points)
        ...
        double_frequency_axis = torch.arange(2 * fft_size_val, device=x.device,
                                             dtype=x.dtype) / fft_size_val * fs_t - fs_t
        ...
        center_frequency = torch.arange(half + 1, device=x.device, dtype=x.dtype) / fft_size_val * fs_t

        low_levels = self.interp1H_torch(double_frequency_axis + delta / 2, double_segment,
                                         center_frequency - current_f0 / 3)
        high_levels = self.interp1H_torch(double_frequency_axis + delta / 2, double_segment,
                                          center_frequency + current_f0 / 3)
        ...
        quefrency_axis = torch.arange(fft_size_val, device=x.device, dtype=x.dtype) / fs_t
        ...
        compensation_lifter = (1 - 2 * q1) + 2 * q1 * torch.cos(2 * torch.pi * current_f0 * quefrency_axis)
    ...
    def interp1H_torch(self, x: torch.Tensor, y: torch.Tensor, xi: torch.Tensor) -> torch.Tensor:
        delta_x = x[1] - x[0]
