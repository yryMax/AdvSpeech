            ...
            for source_file in source_files:
                ...
                if ref_files:
                    self.data.append({
                        ...
                        'ref_files': ref_files,
                        'adv_path': os.path.join(subdir_path, f"{prefix}_{self.output_suffix}.wav")
                    })
...
if __name__ == '__main__':
    ...
    for i, item in enumerate(tqdm(dataloader, desc="Processing Data")):
        ...
        for ref_waveform in ref_waveforms:
            ...
            snr_metric.update(x_adv.cpu().float().detach().unsqueeze(0), ref_waveform.squeeze(0))
            pesq_metric.update(x_adv.cpu().float().detach().unsqueeze(0), ref_waveform.squeeze(0))
            secs_metric.update(x_adv.cpu().float().detach().unsqueeze(0), ref_waveform.squeeze(0))
    ...
    logging.info(f"Final Metrics - SNR: {snr_value}, PESQ: {pesq_value}, SECS: {secs_value}")
