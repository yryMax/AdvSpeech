...
from loss import SpectralFlatnessLoss
...
def get_envelope(audio, sr=22050):
    ...
    f0, time_stamp = pw.dio(
        audio.flatten().numpy().astype(np.float64), sr, frame_period=5.0
    )
    envelope = env_layer(
        torch.tensor(audio.flatten()),
        torch.tensor(f0).double(),
        torch.tensor(time_stamp).double(),
    )
    return envelope
...
def optimize_input(
    ...
    for step in range(num_steps):
        ...
        if not torch.isfinite(loss):
            print(
                f"Optimization stopped at step {step} due to invalid loss (NaN or Inf)."
            )
            break

        if (
            len(loss_history) > 0
            and loss_history[-1] - loss.item() < no_progress_threshold
        ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process audio using SSIM optimization."
    )
    ...
    parser.add_argument(
        "--output", default="output.wav", help="Path to save the processed audio file."
    )
    parser.add_argument(
        "--sr",
        type=int,
    ...
    if args.config:
        ...
        if not args.input or not args.reference:
            parser.error(
                "Either --config or both --input and --reference must be provided."
            )
