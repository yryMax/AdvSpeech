...
def check_data(root_dir, resample_rate=22050):
    ...
    if not subdirs:
        messages.append(
            f"[WARNING] No subdirectories found in '{root_dir}'. Nothing to check."
        )
    ...
    for subdir in subdirs:
        ...
        if not speaker_files_map:
            messages.append(
                f"[ERROR] No valid speaker_i.wav files found in '{speaker_dir}'."
            )
            continue
        ...
        if max_i < 2:
            messages.append(
                f"[ERROR] The maximum i is {max_i}, which is < 2 in '{speaker_dir}'."
            )
        ...
        for i_val, deriv_files in speaker_files_map.items():
            ...
            for dfile in deriv_files:
                ...
                if d_len < base_len:
                    ...
                    print(
                        f"[INFO] '{dfile}' length ({d_len}) > base file '{base_file}' length ({base_len}). "
                    )
